...
function fn_Js_Bridge(arg) {
	...
	if("I" == phoneOS){
		...
		/** SOFTM-BRIDGE-GUARD START 날짜:20260521
		 * 네이티브 브릿지가 없는 팝업/웹 테스트 환경에서 JS 초기화가 중단되지 않도록 방어
		 */
		if (window.callApp && typeof window.callApp.appBridge === "function") {
			window.callApp.appBridge(jsonString);
			return true;
		}
		if (window.console && typeof window.console.warn === "function") {
			window.console.warn("Native Android bridge is not ready.", arg);
		}
		/** SOFTM-BRIDGE-GUARD END 날짜:20260521 */
		return false;
	}
	return false;
}
 ...
 * @date 2016. 07. 13.
 */
/** SOFTM-MAPROUTE START: 구글지도 모드에서는 검색/집회사 화면을 Google 전용 페이지로 연결한다. 2026-05-19 */
function fn_ResolveMapModeUrl(url) {
	var useStoredMapMode = typeof isWeb === "undefined" || isWeb; // SOFTM-MAPROUTE: 앱에서는 네이티브 info를 기준으로 하고 웹 테스트에서만 저장값을 우선한다. 2026-05-19
	var mapMode = (useStoredMapMode ? localStorage.getItem("_MAP_MODE") : "") || (window.info && window.info.mapMode);
	if (!url || String(mapMode) !== "2") return url;
	return String(url)
		.replace(/(^|\/)search\.html(?=([?#]|$))/g, "$1search_google.html")
		.replace(/(^|\/)home_company\.html(?=([?#]|$))/g, "$1home_company_google.html");
}
/** SOFTM-MAPROUTE END */

/** SOFTM-HOMECOMPANY-OVERLAY-HIDE START 날짜:20260521
 * 메뉴에서 집/회사/즐겨찾기 팝업으로 진입할 때 기존 오버레이 자동 재개를 막기 위한 대상 페이지 판별
 */
function fn_ShouldSuppressOverlayAutoResumeForPop(url) {
	var rawUrl = String(url || "");
	var normalizedUrl = rawUrl.split("#")[0].split("?")[0];
	var pageName = normalizedUrl.substring(normalizedUrl.lastIndexOf("/") + 1);
	var queryString = rawUrl.split("#")[0].split("?")[1] || "";
	var isFavoritePage = (pageName === "search.html" || pageName === "search_google.html") &&
		/(^|&)idx=2(&|$)/.test(queryString);
	return pageName === "home_company.html" || pageName === "home_company_google.html" || isFavoritePage; // SOFTM-FAVORITE-OVERLAY-HIDE 날짜:20260521: 메뉴 즐겨찾기 팝업 진입 시 오버레이 자동 재표시 방지
}
/** SOFTM-HOMECOMPANY-OVERLAY-HIDE END 날짜:20260521 */

/** SOFTM-MAPSWITCH START: search/home_company 화면에서 티맵지도와 구글지도 화면을 즉시 전환한다. 2026-05-19 */
function fn_SwitchMapMode(mapMode) {
	var page = location.pathname.split("/").pop();
	var googlePage = page.indexOf("_google.html") >= 0;
	var nextMapMode = mapMode ? String(mapMode) : (googlePage ? "1" : "2");
	var targetMap = {
		"search.html": "search_google.html",
		"search_google.html": "search.html",
		"home_company.html": "home_company_google.html",
		"home_company_google.html": "home_company.html"
	};
	var targetPage = targetMap[page];
	if (!targetPage) return false;
	fn_DeviceSaveData("_MAP_MODE", nextMapMode);
	if (window.info) window.info.mapMode = nextMapMode;
	fn_StopOverlaySafely(); // SOFTM-MAPSWITCH-OVERLAY-HIDE 날짜:20260521: 지도 보기 화면 전환 전 펼침형 오버레이가 화면을 가리지 않도록 중지
	location.href = targetPage + location.search + location.hash;
	return false;
}

/** SOFTM-MAPSWITCH-OVERLAY-HIDE START 날짜:20260521
 * 오버레이 제거 실패가 지도 화면 JS 초기화를 막지 않도록 안전 호출 함수 추가
 */
function fn_StopOverlaySafely() {
	try {
		if (typeof fn_StopOverlay === "function") fn_StopOverlay();
	} catch (e) {
		if (window.console && typeof window.console.warn === "function") window.console.warn("Stop overlay failed.", e);
	}
}
/** SOFTM-MAPSWITCH-OVERLAY-HIDE END 날짜:20260521 */

function fn_InitMapModeSwitchButton() {
	var button = document.getElementById("btnSwitchMapMode");
	if (!button) return;
	var googlePage = location.pathname.split("/").pop().indexOf("_google.html") >= 0;
	var label = googlePage ? fn_MapModeSwitchText("google_navi.switch_to_tmap", "티맵지도 보기") : fn_MapModeSwitchText("google_navi.switch_to_google", "구글지도 보기"); // SOFTM-MAPSWITCH: 지도 전환 버튼 문구를 보기 액션으로 변경. 2026-05-19
	var labelText = label; // SOFTM-MAPSWITCH: 버튼 라벨도 대상 지도명이 아닌 전환 액션 문구로 표시. 2026-05-19
	button.setAttribute("title", label);
	button.setAttribute("aria-label", label);
	var labelElement = button.querySelector(".map-mode-switch-label");
	if (labelElement) labelElement.innerHTML = labelText;
}

function fn_MapModeSwitchText(key, fallback) {
	var translated = null;
	if (typeof googleNaviT === "function") translated = googleNaviT(key);
	else if (typeof t === "function") translated = t(key);
	else translated = fn_MapModeSwitchMessageText(key);
	if (translated && translated !== key) return translated;
	return fallback;
}

function fn_MapModeSwitchMessageText(key) {
	if (typeof messages === "undefined") return "";
	var lngValue = localStorage.getItem("lng") || (navigator.language ? navigator.language.split("-")[0] : "ko");
	lngValue = lngValue === "ko" ? "ko" : "en";
	var source = messages[lngValue] && messages[lngValue].translation ? messages[lngValue].translation : null;
	var parts = String(key || "").split(".");
	for (var i = 0; source && i < parts.length; i++) {
		source = source[parts[i]];
	}
	return typeof source === "string" ? source : "";
}
// SOFTM-I18N: common.js ready 이전에도 지도 전환 버튼이 messages 다국어 값을 읽도록 보완. 2026-05-19

if (document.readyState === "loading") {
	document.addEventListener("DOMContentLoaded", fn_InitMapModeSwitchButton);
} else {
	fn_InitMapModeSwitchButton();
}
/** SOFTM-MAPSWITCH END */

function fn_OpenPop(url, callBackFunc) {
	url = fn_ResolveMapModeUrl(url); // SOFTM-MAPROUTE: 팝업 이동 전 지도 모드별 페이지로 변환. 2026-05-19
		var result = { "header":{ "api":"103" }, "body":{ "url":url, "callBackFunc":callBackFunc, "suppressOverlayAutoResume":fn_ShouldSuppressOverlayAutoResumeForPop(url) } }; // SOFTM-FAVORITE-OVERLAY-HIDE 날짜:20260521: 집/회사/즐겨찾기 메뉴 팝업 진입 시 오버레이 재개 억제값 전달
	//로그인 초시계 초기화처리
//	remaining = timeIni;
	if(CommonUtil.fn_IS_APP() == "I" || CommonUtil.fn_IS_APP() == "A"){
	    fn_Js_Bridge(result);
...
function fn_OpenUrl(url, callBackFunc) {
	url = fn_ResolveMapModeUrl(url); // SOFTM-MAPROUTE: 일반 이동 전 지도 모드별 페이지로 변환. 2026-05-19
