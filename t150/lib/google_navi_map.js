...
(function(window, document) {
  ...
  /*
   * SOFTM-GOOGLE-LOCALE START 날짜:20260521 : Google Maps API language/region을 앱 언어와 기기 국가에 맞춰 설정
   */
  var GOOGLE_LANGUAGE_DEFAULT_REGION = {
    ko: "KR",
    en: "US",
    pt: "BR",
    es: "ES",
  ...
  function getParamValue(name) {
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      if (decodeURIComponent(parts[0] || "") === name) {
        return parts.length > 1 ? decodeURIComponent(parts.slice(1).join("=").replace(/\+/g, " ")) : "";
      }
    }
    return "";
  }

  function browserLocaleParts() {
    var locale = window.navigator && (window.navigator.language || window.navigator.userLanguage) ? (window.navigator.language || window.navigator.userLanguage) : "";
    return localeTagParts(locale);
  }

  function localeTagParts(locale) {
    var parts = String(locale || "").replace("_", "-").split("-");
    return {
      language: String(parts[0] || "").toLowerCase(),
      country: String(parts[1] || "").toUpperCase()
    };
  }

  function normalizeGoogleBaseLanguage(lang) {
    var value = String(lang || "").toLowerCase().replace("_", "-");
    var base = value.split("-")[0];
    if (base === "in") base = "id";
    return Object.prototype.hasOwnProperty.call(GOOGLE_LANGUAGE_DEFAULT_REGION, base) ? base : "en";
  }

  function normalizeGoogleRegion(country) {
    var value = String(country || "").toUpperCase().replace(/[^A-Z]/g, "");
    return value.length === 2 ? value : "";
  }

  function googleLanguageCode(baseLanguage, region) {
    if (baseLanguage === "pt" && region === "BR") return "pt-BR";
    if (baseLanguage === "pt" && region === "PT") return "pt-PT";
    return baseLanguage;
  }

  function googleLocale() {
    var requestedLanguage = getParamValue("lng");
    var requestedParts = localeTagParts(requestedLanguage);
    var browserParts = browserLocaleParts();
    var appLanguage = typeof window.googleNaviLang === "function" ? window.googleNaviLang() : "";
    var storedLanguage = "";
    try {
      storedLanguage = window.localStorage ? window.localStorage.getItem("lng") : "";
    } catch (e) {}
    var storedParts = localeTagParts(storedLanguage);
    var baseLanguage = normalizeGoogleBaseLanguage(requestedLanguage || appLanguage || storedLanguage || browserParts.language);
    var deviceCountry = window.info && window.info.device ? window.info.device.country : "";
    var region = normalizeGoogleRegion(
      getParamValue("country") ||
      requestedParts.country ||
      (requestedLanguage ? GOOGLE_LANGUAGE_DEFAULT_REGION[baseLanguage] : "") ||
      deviceCountry ||
      storedParts.country ||
      browserParts.country ||
      GOOGLE_LANGUAGE_DEFAULT_REGION[baseLanguage]
    );
    if (!region) region = GOOGLE_LANGUAGE_DEFAULT_REGION[baseLanguage] || "US";
    return {
      language: googleLanguageCode(baseLanguage, region),
      region: region
    };
  }
  /*
   * SOFTM-GOOGLE-LOCALE END
   */
  ...
  function loadGoogle(callback) {
    ...
    var locale = googleLocale();
    script.src = "https://maps.googleapis.com/maps/api/js?key=" + encodeURIComponent(key) +
      "&libraries=places&language=" + encodeURIComponent(locale.language) + "&region=" + encodeURIComponent(locale.region) + "&loading=async&callback=SOFTM_GOOGLE_MAPS_READY";
